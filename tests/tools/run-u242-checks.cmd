...
set "LOG=%ROOT%\U242-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-2.4.2 Locale-Aware strftime Migration Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (395 tests, ~9100 assertions)
>>"%LOG%" echo Scope: migrates the last two executable strftime() sites (Configurations.php:582 and :585) onto ProcessMaker\Util\LegacyLocaleDate, and fixes the English branch that rendered Spanish names.
>>"%LOG%" echo Out of scope: mcrypt, FILTER_SANITIZE_STRING, strptime, eval hardening, set_error_handler, any schema or dependency change.
>>"%LOG%" echo.

echo U-2.4.2 offline checks started. Full output: "%LOG%"
...
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-2.4.2 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-2.4.2 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U242_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-2.4.2 offline acceptance completed.
echo Send back U242-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U242_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-2.4.2 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

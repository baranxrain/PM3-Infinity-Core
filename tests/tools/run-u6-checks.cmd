...
set "LOG=%ROOT%\U6-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-6 PMScript Generated String Migration Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (360 tests, ~5450 assertions)
>>"%LOG%" echo Scope: class.pmScript.php:479 generated catch block now emits LegacyUtf8::encode. UTF-8 ratchet 5/0 -^> 4/0.
>>"%LOG%" echo Out of scope: eval() hardening, set_error_handler, and the two locale-aware strftime sites.
>>"%LOG%" echo.

echo U-6 offline checks started. Full output: "%LOG%"
call :run "Locate PHP" "where php"
if errorlevel 1 goto :failed
call :run "PHP version" "php --version"
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-6 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-6 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U6_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-6 offline acceptance completed.
echo Send back U6-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U6_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-6 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

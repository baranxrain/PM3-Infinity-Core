...
set "LOG=%ROOT%\U27-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-2.7 FILTER_SANITIZE_STRING Migration Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (407 tests, assertions may vary)
>>"%LOG%" echo Scope: migrates FILTER_SANITIZE_STRING and FILTER_FLAG_STRIP_LOW/HIGH hits in gulliver\system\class.inputfilter.php.
>>"%LOG%" echo Out of scope: eval hardening, set_error_handler, any schema or dependency change.
>>"%LOG%" echo.

echo U-2.7 offline checks started. Full output: "%LOG%"
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-2.7 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U27_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-2.7 offline acceptance completed.
echo Send back U27-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U27_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-2.7 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

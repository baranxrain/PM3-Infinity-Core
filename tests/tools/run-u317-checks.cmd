...
set "LOG=%ROOT%\U317-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-3.17 Final Executable Eval Closure Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (574 tests, assertions may vary)
>>"%LOG%" echo Scope: route the final two trusted PMScript trigger/condition bodies through an OS-owned temporary include bridge.
>>"%LOG%" echo Out of scope: non-executable JavaScript/comment/string occurrences, schema and dependency changes.
>>"%LOG%" echo.

echo U-3.17 offline checks started. Full output: "%LOG%"
...
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-3.17 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "U-3.17 final executable eval closure preflight" "php tests\tools\verify-u317.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-3.17 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U317_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-3.17 offline acceptance completed.
echo Send back U317-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U317_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-3.17 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

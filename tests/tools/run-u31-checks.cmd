...
set "LOG=%ROOT%\U31-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-3.1 PmDashlet Eval Migration Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (429 tests, assertions may vary)
>>"%LOG%" echo Scope: migrate three PmDashlet eval() sites to call_user_func()/constant().
>>"%LOG%" echo Out of scope: trigger body execution replacement, dynamic model/criteria evals, any schema or dependency change.
>>"%LOG%" echo.

echo U-3.1 offline checks started. Full output: "%LOG%"
...
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-3.1 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-3.1 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U31_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-3.1 offline acceptance completed.
echo Send back U31-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U31_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-3.1 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

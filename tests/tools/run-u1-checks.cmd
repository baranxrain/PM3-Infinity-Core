...
set "LOG=%ROOT%\U1-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-1 Acceptance R5 Offline
...
echo U-1 R5 offline checks started. Full output: "%LOG%"
call :run "Locate PHP" "where php"
if errorlevel 1 goto :failed
call :run "PHP version" "php --version"
if errorlevel 1 goto :failed
call :run "U-1 preflight, PHP 8.1, PHPUnit extensions, ext-phar, and PHAR integrity gate" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Recheck PHPUnit 9.5.8 PHAR integrity before suite" "php tests\tools\verify-u1.php --phar-only"
if errorlevel 1 goto :failed
call :run "Run DB-free U-1 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never --testdox"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U1_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-1 R5 offline acceptance completed.
echo Send back U1-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U1_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-1 R5 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

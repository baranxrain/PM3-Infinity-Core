...
set "LOG=%ROOT%\U231-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-2.3.1 strftime Inventory Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (81 tests, ~4100 assertions)
>>"%LOG%" echo Scope: inventory only. No call site was migrated by U-2.3.1.
>>"%LOG%" echo.

echo U-2.3.1 offline checks started. Full output: "%LOG%"
call :run "Locate PHP" "where php"
if errorlevel 1 goto :failed
call :run "PHP version" "php --version"
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-2.3.1 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "U-2.3.1 record native strftime oracle for this runtime" "php tests\tools\generate-strftime-oracle.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-2.3.1 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never --testdox"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U231_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-2.3.1 offline acceptance completed.
echo Send back U231-ACCEPTANCE.log and tests\fixtures\legacy-strftime-oracle.json for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U231_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-2.3.1 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

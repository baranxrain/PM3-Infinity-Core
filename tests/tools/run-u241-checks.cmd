...
set "LOG=%ROOT%\U241-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-2.4.1 Locale Oracle Capture Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (368 tests, ~5600 assertions)
>>"%LOG%" echo Scope: tooling only. Captures locale-dependent strftime evidence for Configurations.php:582 and :585. No production file changes.
>>"%LOG%" echo Out of scope: migrating either locale-aware call site, eval hardening, set_error_handler.
>>"%LOG%" echo.

echo U-2.4.1 offline checks started. Full output: "%LOG%"
...
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-2.4.1 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-2.4.1 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U241_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-2.4.1 offline acceptance completed.
echo Send back U241-ACCEPTANCE.log and tests\fixtures\legacy-strftime-locale-oracle.json for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U241_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-2.4.1 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

...
set "LOG=%ROOT%\U311-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-3.11 XmlForm Eval Cluster Migration Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (487 tests, assertions may vary)
>>"%LOG%" echo Scope: migrate all seven executable eval sites in class.xmlform.php.
>>"%LOG%" echo Out of scope: remaining Gulliver runtime evals outside XmlForm, trigger body execution, dynamic model/criteria evals, any schema or dependency change.
>>"%LOG%" echo.

echo U-3.11 offline checks started. Full output: "%LOG%"
...
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-3.11 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-3.11 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U311_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-3.11 offline acceptance completed.
echo Send back U311-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U311_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-3.11 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

...
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-3.19 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "U-3.17 executable eval closure preflight" "php tests\tools\verify-u317.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-3.19 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U319_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-3.19 offline acceptance completed.
echo Send back U319-ACCEPTANCE.log for evidence review.
exit /b 0
...
echo U-3.19 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

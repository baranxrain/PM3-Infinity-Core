...
set "LOG=%ROOT%\U2-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-2.1 Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (33 tests, ... assertions)
>>"%LOG%" echo.

echo U-2.1 offline checks started. Full output: "%LOG%"
call :run "Locate PHP" "where php"
if errorlevel 1 goto :failed
call :run "PHP version" "php --version"
if errorlevel 1 goto :failed
call :run "Combined U-1 and U-2.1 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Recheck PHPUnit 9.5.8 PHAR integrity before suite" "php tests\tools\verify-u2.php --phar-only"
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 and U-2.1 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never --testdox"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U2_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-2.1 offline acceptance completed.
echo Send back U2-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U2_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-2.1 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

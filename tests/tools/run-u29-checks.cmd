...
set "LOG=%ROOT%\U29-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-2.9 eval Inventory Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (417 tests, assertions may vary)
>>"%LOG%" echo Scope: inventory/classify the accepted eval() surface; no production migration.
>>"%LOG%" echo Out of scope: eval hardening, production changes, any schema or dependency change.
>>"%LOG%" echo.

echo U-2.9 offline checks started. Full output: "%LOG%"
...
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-2.9 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "U-2.9 eval inventory drift check" "php tests\tools\generate-eval-inventory.php --check"
if errorlevel 1 goto :failed
call :run "U-2.9 eval inventory preflight" "php tests\tools\verify-u29.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-2.9 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U29_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-2.9 offline acceptance completed.
echo Send back U29-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U29_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-2.9 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

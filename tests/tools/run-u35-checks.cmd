...
set "LOG=%ROOT%\U35-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-3.5 fields_Ajax Eval Migration Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (453 tests, assertions may vary)
>>"%LOG%" echo Scope: migrate fields_Ajax CURRENT_PAGE_INITILIZATION execution away from eval().
>>"%LOG%" echo Out of scope: trigger body execution replacement, dynamic model/criteria evals, Gulliver runtime evals, core bootstrap evals, any schema or dependency change.
>>"%LOG%" echo.

echo U-3.5 offline checks started. Full output: "%LOG%"
...
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-3.5 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-3.5 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U35_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-3.5 offline acceptance completed.
echo Send back U35-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U35_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-3.5 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

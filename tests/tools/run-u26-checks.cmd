...
set "LOG=%ROOT%\U26-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-2.6 mcrypt Branch Removal Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (403 tests, assertions may vary)
>>"%LOG%" echo Scope: removes the executable mcrypt_* branch from workflow\engine\classes\Padl.php and pins the regular cipher path.
>>"%LOG%" echo Out of scope: FILTER_SANITIZE_STRING, eval hardening, set_error_handler, any schema or dependency change.
>>"%LOG%" echo.

echo U-2.6 offline checks started. Full output: "%LOG%"
...
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-2.6 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-2.6 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U26_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-2.6 offline acceptance completed.
echo Send back U26-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U26_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-2.6 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%

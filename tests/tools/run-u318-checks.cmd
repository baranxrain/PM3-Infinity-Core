...
set "LOG=%ROOT%\U318-ACCEPTANCE.log"
set "PHPUNIT_PHAR=tests\tools\phpunit-9.5.8.phar"
set "COMPOSER_DISABLE_NETWORK=1"
set "COMPOSER_NO_INTERACTION=1"
cd /d "%ROOT%" || exit /b 2

> "%LOG%" echo ProcessMaker 3.8.3 Community - U-3.18 Client and Raw Eval Closure Acceptance Offline
...
>>"%LOG%" echo Expected PHPUnit result: OK (592 tests, assertions may vary)
>>"%LOG%" echo Scope: remove the final twelve raw eval tokens across generated JavaScript, AJAX callbacks, conditions and comments.
>>"%LOG%" echo Out of scope: schema and dependency changes.
>>"%LOG%" echo.

echo U-3.18 offline checks started. Full output: "%LOG%"
...
if errorlevel 1 goto :failed
call :run "Combined U-1 through U-3.18 harness preflight" "php tests\tools\verify-u1.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "U-3.8 System workspace credential eval migration preflight" "php tests\tools\verify-u38.php"
if errorlevel 1 goto :failed
call :run "U-3.9 Publisher dynamic constructor eval migration preflight" "php tests\tools\verify-u39.php"
if errorlevel 1 goto :failed
call :run "U-3.10 PagedTable eval cluster migration preflight" "php tests\tools\verify-u310.php"
if errorlevel 1 goto :failed
call :run "U-3.11 XmlForm eval cluster migration preflight" "php tests\tools\verify-u311.php"
if errorlevel 1 goto :failed
call :run "U-3.12 Gulliver direct dispatch cluster preflight" "php tests\tools\verify-u312.php"
if errorlevel 1 goto :failed
call :run "U-3.13 Gulliver runtime eval closure preflight" "php tests\tools\verify-u313.php"
if errorlevel 1 goto :failed
call :run "U-3.14 dynamic model dispatch cluster preflight" "php tests\tools\verify-u314.php"
if errorlevel 1 goto :failed
call :run "U-3.15 AdditionalTables criteria closure preflight" "php tests\tools\verify-u315.php"
if errorlevel 1 goto :failed
call :run "U-3.16 expression execution closure preflight" "php tests\tools\verify-u316.php"
if errorlevel 1 goto :failed
call :run "U-3.18 client-side and textual eval closure preflight" "php tests\tools\verify-u317.php"
if errorlevel 1 goto :failed
call :run "U-3.18 client-side and textual eval closure preflight" "php tests\tools\verify-u318.php"
if errorlevel 1 goto :failed
...
if errorlevel 1 goto :failed
call :run "Run DB-free combined U-1 through U-3.18 PHPUnit suite" "php %PHPUNIT_PHAR% --configuration phpunit.xml --testsuite unit --colors=never"
if errorlevel 1 goto :failed

>>"%LOG%" echo.
>>"%LOG%" echo U318_ACCEPTANCE=PASS
>>"%LOG%" echo Finished: %DATE% %TIME%
echo [PASS] U-3.18 offline acceptance completed.
echo Send back U318-ACCEPTANCE.log for evidence review.
exit /b 0
...
>>"%LOG%" echo.
>>"%LOG%" echo U318_ACCEPTANCE=FAIL
>>"%LOG%" echo Failed: %DATE% %TIME%
echo U-3.18 offline acceptance failed. Inspect "%LOG%".
exit /b %FINAL_RC%
